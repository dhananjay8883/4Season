...
mongoose.connect(
  "mongodb+srv://virajsurve:1gy3JDrqYLDptMlc@cluster0.hc9gkg7.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
);
...
app.post("/places", (req, res) => {
  ...
  if (
    ...
    isNaN(maxGuestNumber) ||
    isNaN(priceNumber)
  ) {
    return res
      .status(400)
      .json({ error: "checkIn, checkOut and maxGuest must be numbers" });
  }
...
app.post("/wishlist", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while adding to the wishlist." });
  }
});
...
app.get("/wishlist", async (req, res) => {
  const userData = await getUserDataFromToken(req);
  try {
    const wishlistItems = await Wishlist.find({ userId: userData.id }).populate(
      "place"
    );
...
app.delete("/wishlist/:id", async (req, res) => {
  ...
  try {
    ...
    if (item) {
      ...
      if (item.userId.toString() === userData.id.toString()) {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while removing from the wishlist." });
  }
});
